...
<div class="container mt-4">
    <nav class="navbar navbar-expand-lg navbar-light bg-white rounded border border-gray-100 <?= \Altum\Router::$controller_key == 'index' ? null : 'mb-6' ?>">
        <a
                href="<?= url() ?>"
                        ...
                        <div class="dropdown-menu dropdown-menu-right">
                            <div class="d-flex flex-column flex-lg-row">
                                ...
                                <div class="pr-lg-3">
                                    ...
                                    <?php if(\Altum\Plugin::is_active('email-signatures') || \Altum\Plugin::is_active('aix')): ?>
                                        <a class="dropdown-item <?= in_array(\Altum\Router::$controller, ['Projects', 'ProjectCreate', 'ProjectUpdate']) ? 'active' : null ?>" href="<?= url('projects') ?>"><i class="fas fa-fw fa-sm fa-project-diagram mr-2"></i> <?= l('projects.menu') ?></a>
                                    <?php endif ?>

                                    <?php if(\Altum\Plugin::is_active('email-signatures') && settings()->signatures->is_enabled): ?>
                                        <a class="dropdown-item <?= in_array(\Altum\Router::$controller, ['Signatures', 'SignatureCreate', 'SignatureUpdate']) ? 'active' : null ?>" href="<?= url('signatures') ?>"><i class="fas fa-fw fa-sm fa-file-signature mr-2"></i> <?= l('signatures.menu') ?></a>
                                        <div class="dropdown-divider"></div>
                                    <?php endif ?>

                                    <?php if(\Altum\Plugin::is_active('aix') && settings()->aix->documents_is_enabled): ?>
                                        <a class="dropdown-item" href="<?= url('templates') ?>"><i class="fas fa-fw fa-sm fa-moon mr-2"></i> <?= l('templates.menu') ?></a>

                                        <a class="dropdown-item" href="<?= url('documents') ?>"><i class="fas fa-fw fa-sm fa-robot mr-2"></i> <?= l('documents.menu') ?></a>
                                    <?php endif ?>

                                    <?php if(\Altum\Plugin::is_active('aix') && settings()->aix->images_is_enabled): ?>
                                        <a class="dropdown-item" href="<?= url('images') ?>"><i class="fas fa-fw fa-sm fa-icons mr-2"></i> <?= l('images.menu') ?></a>
                                    <?php endif ?>

                                    <?php if(\Altum\Plugin::is_active('aix') && settings()->aix->chats_is_enabled): ?>
                                        <a class="dropdown-item" href="<?= url('chats') ?>"><i class="fas fa-fw fa-sm fa-comments mr-2"></i> <?= l('chats.menu') ?></a>
                                    <?php endif ?>

                                    <?php if(\Altum\Plugin::is_active('aix') && settings()->aix->transcriptions_is_enabled): ?>
                                        <a class="dropdown-item" href="<?= url('transcriptions') ?>"><i class="fas fa-fw fa-sm fa-microphone-alt mr-2"></i> <?= l('transcriptions.menu') ?></a>
                                    <?php endif ?>

                                    <?php if(\Altum\Plugin::is_active('aix') && settings()->aix->syntheses_is_enabled): ?>
                                        <a class="dropdown-item" href="<?= url('syntheses') ?>"><i class="fas fa-fw fa-sm fa-voicemail mr-2"></i> <?= l('syntheses.menu') ?></a>
                                    <?php endif ?>
                                </div>
                                <?php endif ?>
