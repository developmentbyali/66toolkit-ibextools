...
class ToolsDataConverter extends Controller {
    ...
    public function index() {
        ...
        /* Meta & title */
        Title::set(sprintf(l('tools.tool_title'), sprintf(l('tools.data_converter_tools.name'), l('tools.' . $from), l('tools.' . $to))), true);
        Meta::set_description(sprintf(l('tools.data_converter_tools.description'), l('tools.' . $from), l('tools.' . $to)));
        Meta::set_keywords(sprintf(l('tools.data_converter_tools.meta_keywords'), l('tools.' . $from), l('tools.' . $to)));
        ...
        if(!empty($_POST)) {
            ...
            if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
                ...
                /* Meta & title */
                Title::set(sprintf(l('tools.tool_title'), sprintf(l('tools.data_converter_tools.submission'), $data['input']['input'], l('tools.' . $from), l('tools.' . $to))), true);
        ...
        $view = new \Altum\View('tools/data_converter', (array) $this);

        $this->add_view_content('content', $view->run($data));
    }

    private function convert_data_units($from, $to, $value) {
        // Define the conversion ratio of each unit to bits
        $units_to_bits = require APP_PATH . 'includes/tools/data_converter_units.php';

        // Convert from the original unit to bits
        $value_in_bits = $value * $units_to_bits[$from];

        // Convert from bits to the target unit
        return $value_in_bits / $units_to_bits[$to];
    }
}
