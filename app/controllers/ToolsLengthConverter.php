...
class ToolsLengthConverter extends Controller {
    ...
    public function index() {
        ...
        /* Meta & title */
        Title::set(sprintf(l('tools.tool_title'), sprintf(l('tools.length_converter_tools.name'), l('tools.' . $from), l('tools.' . $to))), true);
        Meta::set_description(sprintf(l('tools.length_converter_tools.description'), l('tools.' . $from), l('tools.' . $to)));
        Meta::set_keywords(sprintf(l('tools.length_converter_tools.meta_keywords'), l('tools.' . $from), l('tools.' . $to)));
        ...
        if(!empty($_POST)) {
            ...
            if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
                ...
                /* Meta & title */
                Title::set(sprintf(l('tools.tool_title'), sprintf(l('tools.length_converter_tools.submission'), $data['input']['input'], l('tools.' . $from), l('tools.' . $to))), true);
        ...
        $view = new \Altum\View(\Altum\Plugin::get('pro-tools')->path . 'views/tools/length_converter', (array) $this, true);

        $this->add_view_content('content', $view->run($data));
    }

    private function convert_data_units($from, $to, $value) {
        $units_to_meters = require \Altum\Plugin::get('pro-tools')->path . 'includes/tools/length_converter_units.php';

        // Convert from the original unit
        $value_in_meters = $value * $units_to_meters[$from];

        // Convert from bits to the target unit
        return $value_in_meters / $units_to_meters[$to];
    }
}
