...
class ToolsWeightConverter extends Controller {
    ...
    public function index() {
        ...
        /* Meta & title */
        Title::set(sprintf(l('tools.tool_title'), sprintf(l('tools.weight_converter_tools.name'), l('tools.' . $from), l('tools.' . $to))), true);
        Meta::set_description(sprintf(l('tools.weight_converter_tools.description'), l('tools.' . $from), l('tools.' . $to)));
        Meta::set_keywords(sprintf(l('tools.weight_converter_tools.meta_keywords'), l('tools.' . $from), l('tools.' . $to)));
        ...
        $view = new \Altum\View(\Altum\Plugin::get('pro-tools')->path . 'views/tools/weight_converter', (array) $this, true);

        $this->add_view_content('content', $view->run($data));
    }

    private function convert_data_units($from, $to, $value) {
        $units_to_grams = require \Altum\Plugin::get('pro-tools')->path . 'includes/tools/weight_converter_units.php';

        // Convert from the original unit
        $value_in_grams = $value * $units_to_grams[$from];

        // Convert from bits to the target unit
        return $value_in_grams / $units_to_grams[$to];
    }
}
